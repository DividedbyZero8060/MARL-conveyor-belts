...
class MaddpgTrainer:
    ...
    def __init__(
        ...
        for _ in range(config.num_agents):
            ...
            target_critic2 = Critic(critic_input_dim, config).to(self._device)
    ...
    def _update(self) -> Optional[Dict[str, float]]:
        ...
        masked_actions = torch.from_numpy(batch.masked_actions).to(self._device)
        ...
        do_actor_update = (self._update_count % self._config.actor_update_delay == 0)

        # ===== Target actions with policy smoothing =====
        with torch.no_grad():
            next_actions = torch.zeros_like(masked_actions)
            for i in range(num_agents):
                a = self._target_actors[i](next_obs[:, i, :])
                noise = torch.randn_like(a) * self._config.policy_smoothing_noise_std
                noise = torch.clamp(
                    noise,
                    -self._config.policy_smoothing_clip,
                    self._config.policy_smoothing_clip,
                )
                smoothed = torch.clamp(a + noise, 0.0, 1.0)
                # Mirror the rollout masking: if next_obs's gate state is non-zero
                # (Committed/Transit/Deployed), force action[0] to 0. Same rule as
                # the Python masker in run() and the C# Activate() guard.
                gate_state_next = next_obs[:, i, IDX_GATE_STATE]  # (B,)
                gate_not_retracted = (gate_state_next != 0.0).float().unsqueeze(-1)  # (B, 1)
                # action[0] is the gate action; other indices (if any) are message floats.
                mask = torch.ones_like(smoothed)
                mask[:, 0:1] = 1.0 - gate_not_retracted
                smoothed = smoothed * mask
                next_actions[:, i, :] = smoothed
        ...
        for i in range(num_agents):
            # ===== Twin critic update with clipped double Q =====
            with torch.no_grad():
                target_q1 = self._target_critics[i](next_critic_input).squeeze(-1)
                target_q2 = self._target_critics2[i](next_critic_input).squeeze(-1)
            ...
            # Critic 1
            current_q1 = self._critics[i](current_critic_input).squeeze(-1)
            critic_loss1 = F.smooth_l1_loss(current_q1, target_q)
            self._critic_optims[i].zero_grad()
            critic_loss1.backward()
            torch.nn.utils.clip_grad_norm_(self._critics[i].parameters(), max_norm=1.0)
            self._critic_optims[i].step()

            # Critic 2 — independent forward pass on the same input
            current_q2 = self._critics2[i](current_critic_input).squeeze(-1)
            critic_loss2 = F.smooth_l1_loss(current_q2, target_q)
            self._critic2_optims[i].zero_grad()
            critic_loss2.backward()
            torch.nn.utils.clip_grad_norm_(self._critics2[i].parameters(), max_norm=1.0)
            self._critic2_optims[i].step()

            critic_losses.append(0.5 * (critic_loss1.item() + critic_loss2.item()))
            mean_qs.append(current_q1.mean().item())

            # ===== Delayed actor and target updates =====
            if do_actor_update:
                my_action = self._actors[i](obs[:, i, :])
                action_slices = []
                ...
                actions_for_actor = torch.stack(action_slices, dim=1)

                actor_critic_input = self._build_critic_input_batch(obs, actions_for_actor)
                # Use critic 1 only for the actor gradient (TD3 convention).
                actor_loss = -self._critics[i](actor_critic_input).mean()
                self._actor_optims[i].zero_grad()
                actor_loss.backward()
                torch.nn.utils.clip_grad_norm_(self._actors[i].parameters(), max_norm=1.0)
                ...
                soft_update(self._target_critics2[i], self._critics2[i], self._config.tau)
        ...
        if actor_losses:
            metrics["actor_loss_mean"] = float(np.mean(actor_losses))
        return metrics
...
if __name__ == "__main__":
    ...
    ## ---- Run two updates so the delayed actor update fires at least once
    losses = trainer._update()
    assert losses is not None, "_update returned None despite buffer being past warmup"
    losses = trainer._update()
    assert losses is not None
    print(f"_update returned losses (after 2 updates): {losses}")
